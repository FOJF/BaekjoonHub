	...
	public static void main (String[] args) throws IOException {
		...
		for (int i = 0; i < tray.length; i++) {
			...
			for (int j = 0; j < tray[i].length; j++) {
				if (input[j] == 1) bfsQ.add(new Point(i,j));
				tray[i][j] = input[j] == 0 ? Integer.MAX_VALUE : input[j];
			}
		}
		...
		for (int i = 0; i < tray.length; i++) {
			for (int j = 0; j < tray[i].length; j++) {
				if (tray[i][j] == Integer.MAX_VALUE) {System.out.println("-1"); return;}
	...
	public static void bfs() {
		...
		while(!bfsQ.isEmpty()) {
			...
			for (int[] dir : delta) {
				...
				if (isValidIdx(nextX,nextY)  && tray[nextX][nextY] > tray[cur.x][cur.y]+1) {
